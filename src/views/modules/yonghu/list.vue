                ...
                <el-row class="ad"
                    ...
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                            ...
                            href="http://localhost:8080/liulangdongwubeihua/upload/yonghuMuBan.xls">批量导入用户数据模板</a>
                        &nbsp;
                        <el-upload v-if="isAuth('yonghu', '导入导出')" style="display: inline-block"
                            action="liulangdongwubeihua/file/upload" :on-success="yonghuUploadSuccess"
...
export default {
    ...
    methods: {
        ...
        deleteHandler(id) {
            try {
                // Ensure dataListSelections exists and is an array before calling map()
                if (!id && (!this.dataListSelections || !Array.isArray(this.dataListSelections) || this.dataListSelections.length === 0)) {
                ...
                this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
                    ...
                    .then(() => {
                        return this.$http({
                            url: `${this.$baseURL}/yonghu/delete`,
                            method: "post",
                            data: ids
                        });
                    })
        ...
        resetPassword(id) {
            //重置密码
            this.$http({
                url: `${this.$baseURL}/yonghu/resetPassword?id=${id}`,
                method: "get",
                // id:id
            }).then(({ data }) => {
                if (data && data.code === 0) {
        ...
        yonghuUploadSuccess(data) {
            let _this = this;
            _this.$http({
                url: `${this.$baseURL}/yonghu/batchInsert?fileName=${data.file}`,
                method: "get"
            }).then(({ data }) => {
                if (data && data.code === 0) {
                    _this.$message({
