        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '标题' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.chongwulingyangName" placeholder="标题"
                            clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad"
                    ...
                    <el-form-item>
                        <el-button v-if="isAuth('chongwulingyang', '新增')" type="success" icon="el-icon-plus"
                            @click="addOrUpdateHandler()">新增</el-button>
                        &nbsp;
                        <el-button v-if="isAuth('chongwulingyang', '删除')" :disabled="dataListSelections.length <= 0"
                            type="danger" icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
                        &nbsp;
                        <el-button v-if="isAuth('chongwulingyang', '报表')" type="success" icon="el-icon-pie-chart"
                            @click="chartDialog()">报表</el-button>
                        &nbsp;
                        <a style="text-decoration:none" class="el-button el-button--success"
                            v-if="isAuth('chongwulingyang', '导入导出')" icon="el-icon-download"
                            href="http://localhost:8080/liulangdongwubeihua/upload/chongwulingyangMuBan.xls">批量导入宠物领养数据模板</a>
                        &nbsp;
                        <el-upload v-if="isAuth('chongwulingyang', '导入导出')" style="display: inline-block"
                            action="liulangdongwubeihua/file/upload" :on-success="chongwulingyangUploadSuccess"
                            :on-error="chongwulingyangUploadError" :show-file-list=false>
                            <el-button v-if="isAuth('chongwulingyang', '导入导出')" type="success"
                                icon="el-icon-upload2">批量导入宠物领养数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('chongwulingyang', '导入导出')" style="display: inline-block"
                            class="export-excel-wrapper" :data="dataList" :fields="json_fields"
                            name="chongwulingyang.xls">
                            <!-- 导出excel -->
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    v-if="isAuth('chongwulingyang', '查看')" :data="dataList" v-loading="dataListLoading"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="chongwulingyangName" header-align="center" label="标题">
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="chongwuTypes"
                        header-align="center" label="宠物类型">
                        <template v-slot="scope">
                            {{ scope.row.chongwuValue }}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="chongwulingyangPhoto" header-align="center" width="200" label="宠物图片">
                        <template v-slot="scope">
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="jieshuTypes"
                        header-align="center" label="是否被认领">
                        <template v-slot="scope">
                    ...
                    <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
                        <template v-slot="scope">
                            <el-button v-if="isAuth('chongwulingyang', '查看')" type="success" icon="el-icon-tickets"
                                size="mini" @click="addOrUpdateHandler(scope.row.id, 'info')">详情</el-button>
                            <el-button v-if="isAuth('chongwulingyang', '修改')" type="primary" icon="el-icon-edit"
                                size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('chongwulingyang', '删除')" type="danger" icon="el-icon-delete"
...
export default {
    ...
    methods: {
        ...
        getDataList() {
            ...
            this.$http({
                url: `${this.$baseURL}/chongwulingyang/page`,
                method: "get",
                params: params
            }).then(({ data }) => {
                if (data && data.code === 0) {
        ...
        deleteHandler(id) {
            try {
                ...
                this.$confirm(`确定进行 [${id ? "删除" : "批量删除"}] 操作?`, "提示", {
                    ...
                    .then(() => {
                        return this.$http({
                            url: `${this.$baseURL}/forum/delete`, // Adjusted back to 'forum/delete' based on your initial code
                            method: "post",
                            data: { ids } // Ensure the correct format for API request
                        });
                    })
                    .then(({ data }) => {
                        if (data && data.code === 0) {
        ...
        chongwulingyangUploadSuccess(data) {
            let _this = this;
            _this.$http({
                url: `${this.$baseURL}/chongwulingyang/batchInsert?fileName=${data.file}`,
                method: "get"
            }).then(({ data }) => {
                if (data && data.code === 0) {
                    _this.$message({
