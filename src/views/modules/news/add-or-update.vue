...
<script>
    ...
    export default {
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`${this.$baseURL}/dictionary/page?page=1&limit=100&sort=&order=&dicCode=news_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`${this.$baseURL}/news/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
