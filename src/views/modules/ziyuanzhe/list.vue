        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '自愿者姓名' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.ziyuanzheName" placeholder="自愿者姓名"
                            clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad"
                    ...
                    <el-form-item>
                        <el-button v-if="isAuth('ziyuanzhe', '新增')" type="success" icon="el-icon-plus"
                            @click="addOrUpdateHandler()">新增</el-button>
                        &nbsp;
                        <el-button v-if="isAuth('ziyuanzhe', '删除')" :disabled="dataListSelections.length <= 0"
                            type="danger" icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
                        &nbsp;
                        <el-button v-if="isAuth('ziyuanzhe', '报表')" type="success" icon="el-icon-pie-chart"
                            @click="chartDialog()">报表</el-button>
                        &nbsp;
                        <a style="text-decoration:none" class="el-button el-button--success"
                            v-if="isAuth('ziyuanzhe', '导入导出')" icon="el-icon-download"
                            href="http://localhost:8080/liulangdongwubeihua/upload/ziyuanzheMuBan.xls">批量导入自愿者数据模板</a>
                        &nbsp;
                        <el-upload v-if="isAuth('ziyuanzhe', '导入导出')" style="display: inline-block"
                            action="liulangdongwubeihua/file/upload" :on-success="ziyuanzheUploadSuccess"
                            :on-error="ziyuanzheUploadError" :show-file-list=false>
                            <el-button v-if="isAuth('ziyuanzhe', '导入导出')" type="success"
                                icon="el-icon-upload2">批量导入自愿者数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('ziyuanzhe', '导入导出')" style="display: inline-block"
                            class="export-excel-wrapper" :data="dataList" :fields="json_fields" name="ziyuanzhe.xls">
                            <!-- 导出excel -->
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    v-if="isAuth('ziyuanzhe', '查看')" :data="dataList" v-loading="dataListLoading"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="ziyuanzheName" header-align="center" label="自愿者姓名">
                        <template v-slot="scope">
                            {{ scope.row.ziyuanzheName }}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="ziyuanzhePhoto" header-align="center" width="200" label="头像">
                        <template v-slot="scope">
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="ziyuanzhePhone" header-align="center" label="手机号">
                        <template v-slot="scope">
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="ziyuanzheEmail" header-align="center" label="电子邮箱">
                        <template v-slot="scope">
                    ...
                    <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
                        <template v-slot="scope">
                            <el-button v-if="isAuth('ziyuanzhe', '查看')" type="success" icon="el-icon-tickets"
                                size="mini" @click="addOrUpdateHandler(scope.row.id, 'info')">详情</el-button>
                            <el-button v-if="isAuth('ziyuanzhe', '修改')" type="primary" icon="el-icon-edit" size="mini"
                                @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('ziyuanzhe', '删除')" type="danger" icon="el-icon-delete" size="mini"
                                @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('ziyuanzhe', '修改')" type="success" icon="el-icon-tickets"
                                size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
