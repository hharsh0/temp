        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad"
                    ...
                    <el-form-item>
                        <el-button v-if="isAuth('chongwu', '新增')" type="success" icon="el-icon-plus"
                            @click="addOrUpdateHandler()">新增</el-button>
                        &nbsp;
                        <el-button v-if="isAuth('chongwu', '删除')" :disabled="dataListSelections.length <= 0"
                            type="danger" icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
                        &nbsp;
                        <el-button v-if="isAuth('chongwu', '报表')" type="success" icon="el-icon-pie-chart"
                            @click="chartDialog()">报表</el-button>
                        &nbsp;
                        <a style="text-decoration:none" class="el-button el-button--success"
                            v-if="isAuth('chongwu', '导入导出')" icon="el-icon-download"
                            href="http://localhost:8080/liulangdongwubeihua/upload/chongwuMuBan.xls">批量导入宠物信息数据模板</a>
                        &nbsp;
                        <el-upload v-if="isAuth('chongwu', '导入导出')" style="display: inline-block"
                            action="liulangdongwubeihua/file/upload" :on-success="chongwuUploadSuccess"
                            :on-error="chongwuUploadError" :show-file-list=false>
                            <el-button v-if="isAuth('chongwu', '导入导出')" type="success"
                                icon="el-icon-upload2">批量导入宠物信息数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('chongwu', '导入导出')" style="display: inline-block"
                            class="export-excel-wrapper" :data="dataList" :fields="json_fields" name="chongwu.xls">
                            <!-- 导出excel -->
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    v-if="isAuth('chongwu', '查看')" :data="dataList" v-loading="dataListLoading"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
                        <template v-slot="scope">
                            <el-button v-if="isAuth('chongwu', '查看')" type="success" icon="el-icon-tickets" size="mini"
                                @click="addOrUpdateHandler(scope.row.id, 'info')">详情</el-button>
                            <el-button v-if="isAuth('chongwu', '修改')" type="primary" icon="el-icon-edit" size="mini"
                                @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('chongwu', '删除')" type="danger" icon="el-icon-delete" size="mini"
...
export default {
    ...
    methods: {
        ...
        deleteHandler(id) {
            try {
                ...
                this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
                    ...
                    .then(() => {
                        return this.$http({
                            url: `${this.$baseURL}/chongwu/delete`,
                            method: "post",
                            data: ids
                        });
                    })
        ...
        chongwuUploadSuccess(data) {
            let _this = this;
            _this.$http({
                url: `${this.$baseURL}/chongwu/batchInsert?fileName=${data.file}`,
                method: "get"
            }).then(({ data }) => {
                if (data && data.code === 0) {
                    _this.$message({
